...
void first_come_first_serve_scheduling (int, process []);
bool check_process_left (int, process []);
int fetch_first_process (int, process []);
...
int main () {
    ...
    first_come_first_serve_scheduling (n, process_array);
    return 0;
}

void first_come_first_serve_scheduling (int process_count, process process_array[process_count]) {
    ...
    while (check_process_left(process_count, process_array)) {
        int first_job_first_process_id = fetch_first_process(process_count, process_array);
        execution_flow[execution_flow_array_incrementor] = first_job_first_process_id;
        execution_flow_array_incrementor++;
        time_counter += process_array[first_job_first_process_id].burst_time;
        process_array[first_job_first_process_id].completion_time = time_counter;
        process_array[first_job_first_process_id].turn_around_time = process_array[first_job_first_process_id].completion_time - process_array[first_job_first_process_id].arrival_time;
        process_array[first_job_first_process_id].waiting_time = process_array[first_job_first_process_id].turn_around_time - process_array[first_job_first_process_id].burst_time;
    }
    printf("First Come First Scheduling Algorithm\n");
...
int fetch_first_process (int process_count, process process_array[process_count]) {
    bool cpu_idle = true;
    int process_id;
